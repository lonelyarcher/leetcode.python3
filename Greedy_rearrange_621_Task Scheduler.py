...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        mt, mc = counter.most_common(1)[0]
        g = [[] for _ in range(mc)]
        p = 0
        for t, c in counter.items():
            for _ in range(c):
                g[p].append(t)
                p = (p + 1) % (mc if c == mc else mc - 1)
        print(g)
        return sum(max(len(g[p]), n + 1) for p in range(len(g) - 1)) + len(g[-1])

class Solution2: 
    # no need to fill the target list, we know it must have groups as most frequency of task
    # so fill (n + 1) * (mostFreq - 1) is the must, then if multiple ties of mostFreq, each one will occupy in last group of mostFreq
    # if we fill the group more than n + 1, which means there is no empty slot, the minimum intervals is len(tasks)
    # so the final answer will be max of ( len(tasks) if no empty,  (n + 1) * (mostFreq - 1) + ties if has empty

    def leastInterval(self, tasks: List[str], n: int) -> int:
        counter = collections.Counter(tasks).most_common()
        mostFreq = counter[0][1]
        ties = len([c for c in counter if c[1] == mostFreq])
        return max(len(tasks), (n + 1) * (mostFreq - 1) + ties)
